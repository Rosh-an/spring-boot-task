...
public class MovieControllerTest {
    ...
    public void setUp(){
        ...
        movie = new Movie();
        movie.setBudget(BigDecimal.valueOf(26000));
        movie.setTitle("Jonny");
        movie.setMovieid(1);
    ...
    public void UpdateMovieSuccess() throws Exception {
        Movie movie = new Movie(2,"Conjuring","Horror",BigDecimal.valueOf(10000));
    ...
    public void DeleteMovieFailure() throws Exception {
        when(movieService.deleteMovie(2)).thenThrow(MovieNotFoundException.class);
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/movie")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(movie)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print());
    ...
    public void GetMovieSuccess() throws Exception {
        when(movieService.getMovies(movie.getTitle())).thenReturn(list);
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/movie")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(movie)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print());
    ...
    public void GetMovieFailure() throws Exception {
        when(movieService.getMovies("Inception")).thenThrow(MovieNotFoundException.class);
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/movie")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(movie)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print());
